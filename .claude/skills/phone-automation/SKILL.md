...
phone-cli ui-tree
# 解析节点 text/resource_id/center_x/center_y 定位目标元素

# 操作（坐标为 0-999 归一化值，必须按屏幕宽高换算，详见下方坐标系约定）
...
`phone-cli` 的所有坐标输入命令（`tap`/`double-tap`/`long-press`/`swipe`）使用 **0-999 归一化坐标**，
而 `ui-tree` 返回的 `center_x`/`center_y`（以及 `bounds` 字符串）是**设备 raw 像素坐标**。
两者**不能直接互传**，必须显式换算，否则会点到错误的位置。

**换算公式：**

```
x_normalized = round(center_x * 999 / screen_width)
y_normalized = round(center_y * 999 / screen_height)
```

`screen_width` / `screen_height` 来自 `phone-cli status` 或 `phone-cli device-info` 的 `screen_size` 字段。

**示例：** 屏幕 1080×2240，节点 `center_x=438, center_y=312`：

```
x = round(438 * 999 / 1080) = 405
y = round(312 * 999 / 2240) = 139
phone-cli tap 405 139
```

**反模式（禁止）：**

```bash
# ❌ 直接把 ui-tree 的 raw 像素传给 tap —— 会被当作 0-999 比例缩放，命中错误位置
phone-cli tap 438 312
```

**自检：** 收到 tap 返回的 `{x, y}` 字段后，确认它接近预期 raw 像素位置；偏差较大时立刻停步，重新换算。
...
- **定位元素**：通过 text/content-desc 找到按钮、输入框，取 `center_x`/`center_y`（raw 像素）并按上文「坐标系约定」换算成 0-999 后再传给 `tap`/`swipe`
...
1. Android: `phone-cli launch <app>` or `phone-cli launch --package <pkg> [--activity <activity>]` → `phone-cli wait-for-app <pkg> --timeout 10` → `phone-cli app-state --package <pkg>`
2. iOS: `phone-cli launch --bundle-id <id>` → `phone-cli wait-for-app --bundle-id <id> --timeout 10`
3. Not foreground: `phone-cli get-current-app`, then inspect ui-tree or screenshot. Android may return either a known app name or the raw package name.
...
2. Android: `phone-cli app-log --package <pkg> --filter crash` or `phone-cli app-log --package <pkg> --filter lifecycle`
...
| `launch` | Launch app | `APP`, `--package`, `--activity`, `--bundle-id`, `--app-path` |
| `get-current-app` | Foreground app | Returns app name or package |
...
| Symptom | Fix |
|---------|-----|
| 修改了源码但功能没生效 | `phone-cli` 是客户端/守护进程架构。改源码后**必须重启** daemon：`phone-cli stop && phone-cli start --device-type adb` |
| `tap`/`swipe` 命中错误位置（偏到屏幕角落或别处） | 把 `ui-tree` 的 raw 像素直接传给了 `tap`。所有坐标命令使用 0-999 归一化坐标，必须按 `x = center_x * 999 / screen_width`、`y = center_y * 999 / screen_height` 换算。详见 Step 6「坐标系约定」 |
