...
```
AI 会话 1: acquire(device_type="ios") ──┐
AI 会话 2: acquire(device_type="ios") ──┤──▶ 中央守护进程 ──▶ 设备 A (iOS, 端口 8100)
AI 会话 3: acquire(device_type="adb") ──┘    (单进程)        设备 B (iOS, 端口 8101)
                                              ▲              设备 C (Android, ADB :5038)
                                              │
                                         AI 只指定类型
                                      daemon 负责选设备、
                                      启动设备、分配端口
...
**AI 会话只指定设备类型（`"ios"` / `"adb"` / `"hdc"`），不指定具体设备 ID。** 设备的发现、选择、启动和端口分配全部由守护进程内部管理。

### 状态

```
acquire(device_type="ios") 调用
    │
    ├── 有空闲设备 ──▶ ACTIVE（活跃）
    │
    └── 无空闲设备 ──▶ QUEUED（排队）──(前一会话释放)──▶ ACTIVE
                                                          │
                                        ┌─────────────────┤
                                        ▼                 ▼
                                    RELEASED          EXPIRED
...
class Session:
    session_id: str         # UUID
    device_type: str        # "ios" | "adb" | "hdc"（AI 指定）
    device_id: str | None   # 守护进程分配的具体设备（AI 不感知）
...
请求设备连接。**只需指定设备类型**，守护进程自动选择/启动具体设备。
...
`device_id` 由守护进程返回，AI 无需关心具体是哪台设备。后续 `operate`/`release` 只使用 `session_id`。
...
AI 会话只传入 `device_type`，守护进程内部完成以下决策：

1. **设备发现**：根据 device_type 调用平台发现命令
   - `"adb"` → `adb devices`
   - `"hdc"` → `hdc list targets`
   - `"ios"` → `tidevice list` / Simulator 列表 / App on Mac 检测
2. **设备选择**：从已发现的设备中选择一台空闲设备
   - 优先分配当前无会话持有的设备
   - 所有设备都被占用 → 会话进入排队状态
3. **设备启动**：如果需要，自动启动设备连接
   - iOS：启动 wdaproxy 子进程
   - ADB：启动独立 adb server
   - HDC：启动独立 hdc server
...
```
acquire(device_type="ios") 到达：

1. 发现阶段：
   discovered = discover_devices("ios")
   → ["iPhone-A", "iPhone-B"]

2. 选择阶段：
   for device_id in discovered:
       slot = device_slots.get(device_id)
       if slot is None:
           → 新设备，创建 DeviceSlot，分配端口，建立连接 → 返回
       if slot.current_session_id is None:
           → 空闲设备，复用已有连接 → 返回

3. 排队阶段（所有设备都被占）：
   → 选择 wait_queue 最短的设备排队
   → 阻塞等待直到该设备释放
```

### 设备发现缓存

发现结果缓存 10 秒，避免频繁调用子进程。缓存按 device_type 独立管理。

### 设备生命周期

```
设备发现 → 首次 acquire 时建立连接（端口分配 + 子进程启动）
                ↓
         会话持有期间保持连接
                ↓
         会话释放 → 连接保留（供下一个 acquire 复用）
                ↓
         所有会话释放 + 空闲超时 → 断开连接，释放端口
```

设备连接不随单个会话释放而断开——保持热连接供后续会话复用，只有长时间无人使用时才真正断开。
...
class PhoneClient:
    ...
    def acquire(self, device_type: str, timeout: float = 300,
                wait: bool = True) -> dict:
        """申请指定类型的设备。

        Args:
            device_type: 设备类型，"ios" | "adb" | "hdc"
            timeout: 空闲超时秒数
            wait: 无空闲设备时是否阻塞等待

        Returns:
            {"session_id": "...", "device_id": "...", "port": 8100, ...}
            device_id 和 port 由守护进程分配，AI 无需关心。
        """
...
```python
client = PhoneClient()
client.acquire(device_type="ios")    # 只指定类型，不指定设备
client.tap(100, 200)                 # 操作
client.screenshot("/tmp/s.png")      # 截图
client.release()                     # 释放
```
...
当会话被排队（该类型所有设备被占）时，`acquire` 调用**阻塞等待**：

1. 客户端发送 `acquire(device_type="ios")` 请求。
2. 守护进程发现所有 iOS 设备都被占用 → 创建状态为 `queued` 的会话，加入 wait_queue 最短的 DeviceSlot。
...
| `no_device_available` | 指定类型下没有发现任何设备 | 404 |
| `device_busy` | 所有设备被占且非阻塞模式 | 409 |
